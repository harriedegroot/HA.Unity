...
namespace HADotNet.Core
{
    ...
    public static class ClientFactory
    {
        ...
        /// <param name="instanceAddress">The Home Assistant base instance address (do not include /api/).</param>
        /// <param name="apiKey">The Home Assistant long-lived access token.</param>
        public static void Initialize(string instanceAddress, string apiKey)
        {
            Initialize(new Uri(instanceAddress), apiKey);
        }
        ...
        public static TClient GetClient<TClient>() where TClient : BaseClient, new()
        {
            return new TClient().Initialize<TClient>(InstanceAddress, ApiKey);
        }
    }
}
